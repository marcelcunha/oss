<x-pages.upsert title='Novo Equipamento'>
    <x-form action="{{ route('devices.store') }}" cancelRoute="{{ route('devices.index') }}">
        <div class="grid lg:grid-cols-6 gap-x-4 gap-y-6">
            ...
            <x-select value="{{ old('type_id') }}" label='Tipo' name='type_id' :options="$types"
            ...
            <x-input-text value="{{ old('serial_number') }}" parent-class='col-span-2' label='Serial' name='serial_number' />
