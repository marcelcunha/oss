<x-pages.upsert title='Editar Orçamento'>
    <x-form
        action="{{ route('budgets.update', $budget) }}" cancelRoute="{{ route('budgets.index') }}" id='budget-form' update>
        <div
                ...
                <x-card
                    ...
                    <div class="grid gap-x-4 gap-y-6">
                        <x-input-text
                            ...
                            value="{{ old('configuration.os', $budget->configuration?->os) }}" />
                        ...
                        <x-form-session
                            ...
                            <x-select
                                ...
                                value="{{ old('configuration.cpu.brand_id', $budget->configuration?->cpu['brand_id']) }}" />
                            <x-input-text
                                label='Modelo' name='configuration[cpu][model]' required
                                value="{{ old('configuration.cpu.model', $budget->configuration?->cpu['model']) }}" />
                        </x-form-session>
                        ...
                        <x-form-session
                            ...
                            <x-select
                                ...
                                value="{{ old('configuration.mobo.brand_id', data_get($budget->configuration, 'mobo.brand_id')) }}" />
                            <x-input-text
                                label='Modelo' name='configuration[mobo][model]'
                                value="{{ old('configuration.mobo.model', data_get($budget->configuration, 'mobo.model')) }}" />
                        </x-form-session>
                        ...
                        <x-form-session
                            ...
                            <x-select
                                ...
                                value="{{ old('configuration.memory.0.brand_id', data_get($budget->configuration, 'memory.0.brand_id')) }}" />
                            <x-input-text
                                label='Modelo' name='configuration[memory][0][model]'
                                value="{{ old('configuration.memory.0.model', data_get($budget->configuration, 'memory.0.model')) }}" />
                            <x-input-text
                                label='Tamanho' name='configuration[memory][0][size]' type='number'
                                value="{{ old('configuration.memory.0.size', data_get($budget->configuration, 'memory.0.size')) }}">
                        ...
                        <x-form-session
                            ...
                            <x-select
                                ...
                                value="{{ old('configuration.storage.0.brand_id', data_get($budget->configuration, 'storage.0.brand_id')) }}" />
                            <x-input-text
                                label='Modelo' name='configuration[storage][0][model]'
                                value="{{ old('configuration.storage.0.model', data_get($budget->configuration, 'storage.0.model')) }}" />
                            <x-input-text
                                label='Tamanho' name='configuration[storage][0][size]' type='number'
                                value="{{ old('configuration.storage.0.size', data_get($budget->configuration, 'storage.0.size')) }}">
                                <x-slot:suffix>GB</x-slot>
                            </x-input-text>
                        </x-form-session>
                        ...
                        <x-form-session
                            ...
                            <x-select
                                ...
                                value="{{ old('configuration.gpu.brand_id', data_get($budget->configuration, 'gpu.brand_id')) }}" />
                            <x-input-text
                                label='Modelo' name='configuration[gpu][model]'
                                value="{{ old('configuration.gpu.model', data_get($budget->configuration, 'gpu.model')) }}" />
                            <x-input-text
                                label='Ram' name='configuration[gpu][ram]' type='number'
                                value="{{ old('configuration.gpu.ram', data_get($budget->configuration, 'gpu.ram')) }}">
                                <x-slot:suffix>GB</x-slot>
                            </x-input-text>
                        </x-form-session>
                        ...
                        <x-form-session
                            ...
                            <x-select
                                ...
                                value="{{ old('configuration.power_supply.brand_id', data_get($budget->configuration, 'power_supply.brand_id')) }}" />
                            <x-input-text
                                label='Modelo' name='configuration[power_supply][model]'
                                value="{{ old('configuration.power_supply.model', data_get($budget->configuration, 'power_supply.model')) }}" />
                            <x-input-text
                                label='Potência Nominal' name='configuration[power_supply][wattage]'
                                value="{{ old('configuration.power_supply.wattage', data_get($budget->configuration, 'power_supply.wattage')) }}">
            ...
            <x-textarea
                label='Relatos' name='description' parent-class='lg:col-span-6' value="{{ old('description', $budget->description) }}" />
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('budgetForm', () => ({
                    ...
                    client: @json(old('client_id', $budget->client_id)),
                    device_id: null,
                    deviceResource: null,
                    devices: [],
                    async fillDevices() {
                        try {
                            ...
                            this.device_id = @json(old('device_id', $budget->device_id));
                        } catch (error) {
                            console.error(error);
                        }
                    },
